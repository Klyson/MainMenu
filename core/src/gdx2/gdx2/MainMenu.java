...
public class MainMenu extends InputAdapter implements Screen {
	...
	public MainMenu(final Game1 _game) {
		...
		camera.setToOrtho(false, 800, 480);
                SongOne = new Texture("SongOne.jpg");               
	...
	if (button == Input.Buttons.LEFT && screenX < Gdx.graphics.getWidth() / 2 + 30 && 
                screenX > Gdx.graphics.getWidth() / 2 - 30 && 
                screenY < Gdx.graphics.getHeight() / 2 + 30 && 
                screenY < Gdx.graphics.getHeight() / 2 - 30) {
            isSong1 = true;
        } else if (button == Input.Buttons.LEFT && screenX < Gdx.graphics.getWidth() / 2 + 30 && 
                screenX > Gdx.graphics.getWidth() / 2 - 30 && 
                screenY < 130 && screenY > 70) {	
            isExit = true;
        }
            return true;
	}
